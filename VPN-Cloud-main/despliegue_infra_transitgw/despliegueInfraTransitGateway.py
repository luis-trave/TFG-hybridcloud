...
command = "aws cloudformation create-stack --stack-name {} --template-body file://VPCnubeTransitGateway.yml --tags Key=OPI-Code,Value=008_807102 --capabilities CAPABILITY_IAM --role-arn {}".format(nombre_stack_nube, nombre_rol_iam)
subprocess.run(command, shell=True, check=True)
...
def deploy_vpc_stacks(num_vpcs: int):
    ...
    for i in range(num_vpcs - 1):
        stack_name_extra = f"{nombre_stack_nube}{i+2}"
        cidr = cidr_blocks[i]
        private_subnet_cidr = private_subnet_cidrs[i]
        public_subnet_cidr = public_subnet_cidrs[i]

        cmd = "aws cloudformation create-stack --stack-name {} --template-body file://VPCnubeExtra.yml --tags Key=OPI-Code,Value=008_807102 --capabilities CAPABILITY_IAM --role-arn {} --parameters ParameterKey=VpcCIDR,ParameterValue={} ParameterKey=PublicSubnetCIDR,ParameterValue={} ParameterKey=PrivateSubnetCIDR,ParameterValue={}".format(stack_name_extra, nombre_rol_iam, cidr, public_subnet_cidr, private_subnet_cidr)
